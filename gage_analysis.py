...
def baseflow_correlation_search(climate_dir, in_json, out_json):
    ...
    for csv in l:
        try:
            ...
            qb = df[df.index.month.isin(recession_months)]['qb'].dropna().resample('A').agg(DataFrame.sum,
                                                                                            skipna=False)
            years = [x.year for x in qb.index]
            corr = (0, 0.0)
            found_sig = False
            for lag in offsets:
                dates = [(date(y, recession_months[0], 1), date(y, recession_months[0], 1) + rdlt(months=-lag)) for y in
                         years]
                ...
                if abs(r) > corr[1]:
                    corr = (lag, abs(r))
                    d = {'STANAME': s_meta['STANAME'], 'qb_month': m,
            ...
            if not found_sig:
                print('no significant r at {}'.format(sid, s_meta['STANAME']))
                continue
...
if __name__ == '__main__':
    dq = '/media/research/IrrigationGIS/gages/hydrographs/daily_q_bf'
    ...
    # o_json = '/media/research/IrrigationGIS/gages/station_metadata/basin_lag_recessoion.json'
