...
if __name__ == '__main__':
    ...
    models = os.path.join(root, 'ept_model_zoran_5APR2023')

    if not os.path.exists(models):
        os.mkdir(models)

    validation_csv = os.path.join(root, 'ept_validation_csv')
    rasters = os.path.join(root, 'full_stack')
    out_rasters = os.path.join(root, 'full_stack_pred_gs')
    ...
    nn = DNN(month=m, label=t, _dir=model_dir)
    nn.model_name = MODEL_NAME.format(m)
    # nn.train(2560, data, epochs=1000, verbose=1, save_test_data=validation_data)
    # nn.save()
    nn.load()
    nn.infer_local(rasters, out_rasters, overwrite=False)
# ========================= EOF ====================================================================
