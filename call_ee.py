...
def reduce_classification(tables, years=None, description=None, min_years=0):
    ...
    for yr in years:
        ...
        et = et_coll.mosaic().mask(mask)

        s, e = '{}-01-01'.format(yr), '{}-12-31'.format(yr)
        proj = et.projection().getInfo()
        gridmet = ee.ImageCollection("IDAHO_EPSCOR/GRIDMET").filterDate(s, e).select('pr', 'etr')
        gridmet_red = ee.Reducer.sum()
        gridmet_sums = gridmet.select('pr', 'etr').reduce(gridmet_red).rename(
            'ppt_{}'.format(yr), 'pet_{}'.format(yr)).resample('bilinear').reproject(crs=proj['crs'],
                                                                                     scale=30)

        area = ee.Image.pixelArea()
        ppt = gridmet_sums.select('ppt_{}'.format(yr)).multiply(0.001).multiply(area).rename('ppt_{}'.format(yr))
        pet = gridmet_sums.select('pet_{}'.format(yr)).multiply(0.001).multiply(area).rename('pet_{}'.format(yr))
        crop_cons = et.subtract(gridmet_sums.select(
            'ppt_{}'.format(yr))).rename('cc_{}'.format(yr)).multiply(0.001).multiply(area)
