...
RF_ASSET = 'users/dgketchum/IrrMapper/version_2'
...
def extract_gridmet(tables, years=None, description=None, min_years=0):
    ...
    for yr in years:
        ...
        out_desc = '{}_wy_v2_5AUG2021_{}'.format(description, yr)
        task = ee.batch.Export.table.toCloudStorage(
            data,
            description=out_desc,
            bucket='wudr',
...
if __name__ == '__main__':
    is_authorized()
    # extract_gridmet(BASINS, years=[i for i in range(1991, 2021)], description='basins', min_years=3)
